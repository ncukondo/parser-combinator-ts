...
const sepBy1 = <T>(sepLike:ParserLike<T>) => <U>(parserLike:ParserLike<U>) => {
...
const chain = <R,T extends ParserLike<R>,U>(func:(value:ParserValue<T>)=>Parser<U>) =>(parserLike:T)=>{
  const parser = toParser(parserLike);
...
const assert = <T>(func:(value:T)=>boolean, desc="") => (parserLike:ParserLike<T>) =>{
  const parser = toParser(parserLike);
  parser.pipe(
    chain(value=>func(value) ? ok(value) : fail(desc))
  )
}

const not = <T>(notParser:ParserLike<T>,desc="") => <U>(parserLike:ParserLike<U>)=>{
  const parser = toParser(parserLike);
...
const wrap = <L,R>(leftLike:ParserLike<L>, rightLike?:ParserLike<R>) => <U>(parserLike:ParserLike<U>) =>{
  const left = toParser(leftLike);
  const right = rightLike ? toParser(rightLike) : left;
  const parser = toParser(parserLike);
  return pipe(seq(left,parser,right),map(([,center])=>center));
};

const trim = <L,R>(leftLike:ParserLike<L>, rightLike?:ParserLike<R>) => <U>(parserLike:ParserLike<U>) =>{
...
function times<U>(min:number, max:number):(parserLike:ParserLike<U>) =>Parser<U[]>; 
function times<U>(c1:number,c2?:number):(parserLike:ParserLike<U>) =>Parser<U[]> {
...
const premap = (fn:(input:string)=>string) => <U>(parserLike:ParserLike<U>)=>{
  const parser = toParser(parserLike);
  return makeParser((input, i) =>{
...
const node = (name:string) => <U>(parserLike:ParserLike<U>):Parser<Node<U>>=>{
  const parser = toParser(parserLike);
  return pipe(
      seq(index, parser, index), 
...
const notFollowedBy = <T>(notParser:ParserLike<T>) => <U>(parserLike:ParserLike<U>)=>{
  const parser = toParser(parserLike);
  const _notParser = toParser(notParser);
...
const followedBy = <T>(followParserLike:ParserLike<T>) => <U>(parserLike:ParserLike<U>)=>{
  const parser = toParser(parserLike);
...
export {map,desc,join,sepBy,sepBy1,many,followedBy,not,notFollowedBy,node
,mark,skip,premap,atLeast,atMost,times,or,trim,wrap,fallback,assert,chain}