...
export type ParseFn<T> = (
    ...
    makeFail:(index:number, expected:string)=>FailResult)=>ParseResult<T>;
export type Index = {
  offset:number,
  line:number,
  column:number
...
const fail = (expected:string) =>makeParser((input, i)=>   makeFail(i, expected));
...
export {Parser,string,makeOk,makeFail,makeParser,ok,
  fail,regexp,
