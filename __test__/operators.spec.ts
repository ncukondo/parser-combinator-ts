...
describe("operators", () => {
  ...
  test("remap", () => {
    ...
    expect(remapTest.tryParse("bbbaaa")).toEqual({
      key1: "aaa",
      key2: "tokey2",
    });

    expect(remapTest2.tryParse("aaabbbb")).toEqual({
      key1: "bbbb",
      key2: "aaa",
    });
    expect(remapTest3.tryParse("aaabbbb")).toEqual(["bbbb", "aaa"]);
    expect(remapTest4.tryParse("aaabbbb")).toEqual({
      key1: "bbbb",
      key2: "aaa",
    });
  });
  test("toObj", () => {
    ...
    const toObjTest1 = pipe(
      seq("aaa", "bbb", "ccc"),
      toObj("key1", "", "key3")
    );
    const toObjTest2 = pipe(seq("aaa", "bbb", "ccc"), toObj2);
    expect(toObjTest1.tryParse("aaabbbccc")).toEqual({
      key1: "aaa",
      key3: "ccc",
    });
    expect(toObjTest2.tryParse("aaabbbccc")).toEqual({
      key1: "aaa",
      key2: "bbb",
    });
  });
